...
Netlify can host the frontend from `public/` using `netlify.toml`. Set the Netlify environment variable `API_BASE_URL` to the public URL of the Node backend. Set the backend `FRONTEND_ORIGIN` to `https://sirisia.netlify.app`. Netlify cannot run the downloader's long-running Node/yt-dlp process as a static site.
