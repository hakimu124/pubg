...
Netlify hosts the frontend from `public/`. Set the Netlify environment variable `API_BASE_URL` to the public URL of the Node backend. Deploy the backend on a Node-capable host such as Render using `render.yaml`, and set its `FRONTEND_ORIGIN` to `https://sirisia.netlify.app`. Netlify cannot run the downloader's long-running Node/yt-dlp process as a static site.
